...
extension TransitRealtime_TfnswVehicleDescriptor: SwiftProtobuf.Message, SwiftProtobuf._MessageImplementationBase, SwiftProtobuf._ProtoNameProviding {
  ...
  func traverse<V: SwiftProtobuf.Visitor>(visitor: inout V) throws {
    if let v = self._airConditioned {
      try visitor.visitSingularBoolField(value: v, fieldNumber: 1)
    }
    if let v = self._wheelchairAccessible {
      try visitor.visitSingularInt32Field(value: v, fieldNumber: 2)
    }
    if let v = self._vehicleModel {
      try visitor.visitSingularStringField(value: v, fieldNumber: 3)
    }
    if let v = self._performingPriorTrip {
      try visitor.visitSingularBoolField(value: v, fieldNumber: 4)
    }
    if let v = self._specialVehicleAttributes {
      try visitor.visitSingularInt32Field(value: v, fieldNumber: 5)
    }
    try unknownFields.traverse(visitor: &visitor)
  }
